            ...
            @if ($products->isEmpty())
                <div class="col-12 ">
                    <p class="alert alert-info w-100 text-center my-3">{{ __('user.noProduct') }}
                        <br> {{ __('user.click') }}
                        <a href="{{ route('products.create') }}" class="text-decoration-none">{{ __('user.here') }}</a>
                        {{ __('user.addProduct') }}
                    </p>
                </div>
            @else
                        ...
                        <div class="col-12 col-sm-12 col-md-6 col-lg-4 d-flex border-0">
                            <div class="card h-100 border-0 shadow-sm">
                                <div class="position-relative">
                                    <div id="productImageCarouselCollapse{{ $product->id }}" class="carousel slide"
                                        ...
                                        <button class="carousel-control-prev" type="button"
                                            data-bs-target="#productImageCarouselCollapse{{ $product->id }}"
                                            data-bs-slide="prev">
                                ...
                                <div class="card-body p-3 d-flex flex-column">
                                    <h5 class="card-title fw-bold mb-2 text-truncate">
                                        <a href="{{ route('products.show', $product->id) }}"
                                            class="text-decoration-none"> {{ Str::limit($product->title, 20) }} </a>
                                    </h5>
                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                        <span class="fw-bold fs-5 text-blu">€
                                        ...
                                        <div class="bg-light p-3 rounded mb-3 text-center">
                                            ...
                                            @else
                                                <img src="https://picsum.photos/seed/{{ rand(1, 1000) }}/500"
                                                    ...
                                                    alt="User Avatar">
                                            @endif <a
                                                href="{{ route('personalArea', $product->user->id) }}"
                                    ...
                                    <div class="mt-auto d-flex justify-content-center align-items-center gap-2">
                                        @if (Auth::id() == $product->user_id || Auth::user()->is_revisor == 1)
                                            ...
                                            <div class="modal fade" id="modalDeleteProduct{{ $product->id }}"
                                                data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1"
                                                aria-labelledby="modalDeleteProductLabel{{ $product->id }}"
                                                aria-hidden="true">
                                                <div class="modal-dialog modal-dialog-centered">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h1 class="modal-title fs-4 text-center"
                                                                id="modalDeleteProductLabel{{ $product->id }}">
                                                                {{ __('user.deleteArticle') }} </h1>
                                                            <button type="button" class="btn-close"
                                                                data-bs-dismiss="modal" aria-label="Close"></button>
                                                        </div>
                                                        <div class="modal-footer justify-content-start">
                                                            <button type="button" class="btn btn-base"
                                                                data-bs-dismiss="modal"> {{ __('user.goBack') }}
                                                            </button>
                                                            <form
                                                                action="{{ route('products.destroy', $product->id) }}"
                                                                method="POST"> @csrf @method('DELETE') <button
                                                                    type="submit"
                                                                    class="btn btn-rosso">{{ __('revisor.delete') }}</button>
                                                            </form>
                                                        </div>
                                                    </div>
                                                </div>
