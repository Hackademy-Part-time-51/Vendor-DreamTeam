<div class="col-12 col-md-4 col-lg-3 sidebar-scroll h-100 bg-light border-end d-flex flex-column p-0"
        ...
        @foreach ($chats as $i => $chat)
            @if ($chat['product']['user_id'] === Auth::user()->id || $chat['product']['user_id'] === $chat['user']['id'])
                <button wire:click="selectChat({{ $chat['product']['id'] }},{{ $chat['user']['id'] }})"
                    class="nav-link text-start d-flex align-items-center {{ $i === 0 ? 'active' : '' }}"
                    ...
                    <div >
                        <div class="fw-bold">{{ $chat['user']['name'] }} <span class=" badge bg-success text-end">{{ $chat['msgNotRead'] }}</span></div>
